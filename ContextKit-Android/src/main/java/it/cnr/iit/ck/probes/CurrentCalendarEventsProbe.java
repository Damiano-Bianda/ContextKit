...
class CurrentCalendarEventsProbe extends ContinuousProbe {
    ...
    public String[] getLogFileHeaders() {
        return getContext().getResources().getStringArray(R.array.current_calendar_events_log_file_headers);
    }

    @Override
    public void exec() {
        LoggableElements loggableElements = new LoggableElements(CalendarController.getCurrentEvents(getContext()));
        logOnFile(true, loggableElements);
    }
}
