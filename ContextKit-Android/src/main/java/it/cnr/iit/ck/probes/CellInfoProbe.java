...
class CellInfoProbe extends ContinuousProbe {
    ...
    public String[] getLogFileHeaders() {
        return getContext().getResources().getStringArray(R.array.cell_info_log_file_headers);
    }

    @Override
    public void exec() {
        List<CellInfo> visibleCells = CellController.getVisibleCells(getContext());
        CellsInfo data = new CellsInfo(visibleCells);
        logOnFile(true, data);
    }
}
